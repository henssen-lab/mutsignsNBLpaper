...
###    3RD SCRIPT: PLOT MUTATIONAL SIGNATURE EXPOSURES     ###
##############################################################
...
exp_counts <- read.delim("/data/gpfs-1/users/rodrigue_c/work/landscape_NBL_project/wgs_analysis/sv_Palimpsest_signatures_analysis/SV_signatures_ALLpatients_2022.06.15/exposure_SV_denovo+SIGNAL_signature_absexp_per_patient_COUNT_22.06.13.txt", sep="\t")
exp_freq <- read.delim("/data/gpfs-1/users/rodrigue_c/work/landscape_NBL_project/wgs_analysis/sv_Palimpsest_signatures_analysis/SV_signatures_ALLpatients_2022.06.15/exposure_SV_denovo+SIGNAL_signature_absexp_per_patient_FREQ_22.06.13.txt", sep="\t")
...
ggsave(filename="/data/gpfs-1/users/rodrigue_c/work/landscape_NBL_project/wgs_analysis/sv_Palimpsest_signatures_analysis/SV_signatures_ALLpatients_2022.06.15/plots/Plot_ALLSVS_COSMICsignatures_absexposure_COUNTSPERPATIENT_all114patients_22.06.13.pdf", width = 20, height = 5, plot=PLOT2)
...
ggsave(filename="/data/gpfs-1/users/rodrigue_c/work/landscape_NBL_project/wgs_analysis/sv_Palimpsest_signatures_analysis/SV_signatures_ALLpatients_2022.06.15/plots/Plot_ALLSVS_COSMICsignatures_absexposure_FREQPERPATIENT_all114patients_22.06.13.pdf", width = 20, height = 5, plot=PLOT2)
...
PLOT3 <- PLOT2 + geom_col(position = position_stack(reverse = TRUE)) +scale_y_continuous(name="# of SVs caused by the signature / num of samples") + scale_x_discrete(name="Risk Group") + labs(title = "Counts of COSMIC signatures by risk group (corrected by number of samples)" ,subtitle = "Peifer+Berlin cohorts" ) + guides(fill = guide_legend(title = "Signature")) + theme_classic()
ggsave(filename="/data/gpfs-1/users/rodrigue_c/work/landscape_NBL_project/wgs_analysis/sv_Palimpsest_signatures_analysis/SV_signatures_ALLpatients_2022.06.15/plots/Plot_ALLSVS_absexp_SVS_SIGNAL+denovo_signatures_COUNTS_RISKGROUP_22.06.13.pdf", width = 10, height = 7, plot=PLOT3)
...
#plot with percentage
PLOT2 <- ggplot(input_plot_exp2, aes(x=risk_group,y=value, fill=variable))
PLOT3 <- PLOT2 + geom_col(position = position_fill(reverse = TRUE)) + scale_y_continuous(name="% of SVs caused by the signature") + scale_x_discrete(name="Risk Group") + labs(title = "Percentage of COSMIC signatures by risk group" ,subtitle = "Peifer+Berlin cohorts" ) + guides(fill = guide_legend(title = "Signature")) + theme_classic()
ggsave(filename="/data/gpfs-1/users/rodrigue_c/work/landscape_NBL_project/wgs_analysis/sv_Palimpsest_signatures_analysis/SV_signatures_ALLpatients_2022.06.15/plots/Plot_ALLSVS_absexp_ISVS_SIGNAL+denovo_signatures_FREQ_RISKGROUP_22.06.13.pdf", width = 10, height = 7, plot=PLOT3)
