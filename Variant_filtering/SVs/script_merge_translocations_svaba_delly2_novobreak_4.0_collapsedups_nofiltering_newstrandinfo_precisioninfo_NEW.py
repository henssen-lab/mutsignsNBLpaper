...
input_file_delly2 = sys.argv[2]
input_file_novobreak = sys.argv[3]
 ...
 for res3 in range(0,len(delly2_results)-1):
  if len(re.findall(r'#',delly2_results[res3]))>0:
   ...
   if len(re.findall('BND',res3_line[2]))>0:
    ...
    if res3_line[0] != res3_line_chrom2:
...
results_patient = open('merge_translocations_diffvariantcallers_' + f_name_svaba + '_' + f_name_delly2 + '_' + f_name_novobreak + '_' + 'patient_' + patient_id + '_noVCFdups_collapsedups_nofilters_newinfostrands_precisioninfo.txt','w')
